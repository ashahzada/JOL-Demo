...
public class AgentModel extends BaseAgentModel {
	...
	private Map<String,Boolean> deskStatus = new HashMap<String,Boolean>();
	...
	public AgentModel(final World world) {
		...
		deskStatus.put("MeetingDesk",false);
		deskStatus.put("KitchenTable",false);
		deskStatus.put("DemoDesk",false);
	...
	private void createWorker(final ArrayList<Agent> people,
		...
		while (deskIt.hasNext()) {
			...
			Agent a =
					new Agent(type + "-" + i, door.getPos(), "HumanBlue", world);
			a.setVisible(false);
			EasyTime startWork =
					new EasyTime(AVERAGE_WORK_START).blur(WORK_START_BLUR);

			EasyTime endWork =
					new EasyTime(startWork).shift(AVERAGE_WORK_HOURS, 0).blur(WORK_END_BLUR);

			EasyTime toiletInterval =
					new EasyTime(AVERAGE_TOILET_INTERVAL).blur(TOILET_BLUR);
			...
			people.add(a);
			//System.out.println(a.getName()+" is assigend the desk: "+desk.getName()+" at position: "+desk.getPos().getRow()+ " "+desk.getPos().getCol());
			deskStatus.put(a.getName(), false);
			i++;
		}
	}
	...
	private void beAtLunch(final Agent a) {
		if (((Text) a.get(EVENT)).toString().equalsIgnoreCase(
			"LunchFinished")) {
			a.set(EVENT, new Text("None"));
			deskStatus.put("KitchenTable", false);
	...
	private void beAtDemo(final Agent a) {
		if (((Text) a.get(EVENT)).toString().equalsIgnoreCase(
			"DemoFinished")) {
			a.set(EVENT, new Text("None"));
			deskStatus.put("DemoDesk", false);
	...
	private void goToLunch(final Agent a) {
		a.setImage("HumanBrown");
		a.setDestination((Place) a.get("TemporaryDestination"));
		a.set(ACTIVITY, Activity.GOING_2_LUNCH);
		deskStatus.put(a.getName(), false);
	}
	
	private void goToDemo(final Agent a) {
		a.setImage("HumanBrown");
		a.setDestination((Place) a.get("TemporaryDestination"));
		a.set(ACTIVITY, Activity.GOING_2_DEMO);
		deskStatus.put(a.getName(), false);
	}
	...
	private void handleEvent(final Agent a) {
		...
		if (event.equalsIgnoreCase("ConferenceRoomMeeting")) {
			goToGlobalMeeting(a);
			deskStatus.put("MeetingDesk", true);
		}
		
		else if (event.equalsIgnoreCase("LunchTime")) {
			goToLunch(a);
			deskStatus.put("KitchenTable", true); //To be corrected !
		}
		
		if (event.equalsIgnoreCase("DemoTime")) {
			goToDemo(a);
			deskStatus.put("DemoDesk", true); //To be corrected !
		}
	}
	...
	private void lineInToilet(final Agent a, final EasyTime now) {
		...
		if (!(info instanceof Place)) {
			try {
				toilet =
						world.getNearestPlaceOfType("Bathroom", a.getPos());
	...
	private void goToDesk(final Agent a) {
		...
		a.setVisible(true);
		System.out.println("Setting: Go to Desk");
		System.out.println("Desk destination: "+((Place) a.get(DESK)).getName());
	...
	public void exchangeData() throws IOException, PlaceTypeUndefinedException {
		//System.out.println("In Siafu Data Exchange");
		int flaWri = 0;

		int deskCount = world.getPlacesOfType("StaffDesk").size()+3;
		deskCount += world.getPlacesOfType("StudentDesk").size();
		// Number of variables to be exchanged

		// Arrays that contain the variables to be exchanged
		boolean[] blValWri= new boolean[deskCount];
		//boolean[] blValRea= new boolean[nblRea];
		int i;
		...
		double simTimWri = 0;
				...
				ArrayList<String> keys = new ArrayList<String>(deskStatus.keySet());
				Collections.sort(keys);
				for(i=0; i < deskCount; i++){
					System.out.println("deskname: "+keys.get(i));
					blValWri[i]= deskStatus.get(keys.get(i));
				}
				...
				System.out.println("Siauf is sending: "+strBuf.toString());
				wr.flush();
