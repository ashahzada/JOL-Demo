...
import static de.nec.nle.siafu.office.Constants.LUNCH_START;
import static de.nec.nle.siafu.office.Constants.LUNCH_DURATION_BLUR;
import static de.nec.nle.siafu.office.Constants.LUNCH_DURATION;
import static de.nec.nle.siafu.office.Constants.DEMO_START;
import static de.nec.nle.siafu.office.Constants.DEMO_DURATION_BLUR;
import static de.nec.nle.siafu.office.Constants.DEMO_DURATION;
...
public class WorldModel extends BaseWorldModel {
	...
	/** Beginning of the lunch. */
	private EasyTime lunchStart;

	/** Time at which the lunch ends. */
	private EasyTime lunchEnd;
	
	/** Beginning of the demo. */
	private EasyTime demoStart;

	/** Time at which the demo ends. */
	private EasyTime demoEnd;
	...
	public void doIteration(final Collection<Place> places) {
		...
		try {
			...
			if (lunchStart != null && now.isAfter(lunchStart)) {
				lunchStart = null;
				organizeLunch();
			}
			...
			if (demoStart != null && now.isAfter(demoStart)) {
				demoStart = null;
				organizeDemo();
			}

			if (now.isAfter(demoEnd)) {
				finishDemo();
			}
	...
	/**
	 * Finish the lunch by changing the event in the involved agents.
	 */
	private void finishLunch() {
		System.out.println("Lunch is Finished");
		Iterator<Agent> peopleIt = world.getPeople().iterator();
		while (peopleIt.hasNext()) {
			Agent p = peopleIt.next();
			if (((Text) p.get(EVENT)).toString().equalsIgnoreCase(
				"LunchTime")) {
				p.set(EVENT, new Text("LunchFinished"));
			}
		}
	}

	/**
	 * Invite as many peolpe as there are seats to the kitchen.
	 */
	private void organizeLunch() {
		Collection<Place> seats;
		try {
			seats = world.getPlacesOfType("KitchenSeat");
		} catch (PlaceNotFoundException e) {
			throw new RuntimeException(e);
		}
		Iterator<Place> seatsIt = seats.iterator();
		...
		while (peopleIt.hasNext()) {
			Agent p = peopleIt.next();
			if (((Text) p.get(TYPE)).toString().equalsIgnoreCase("Student")
					&& seatsLeft > 0) {
				seatsLeft--;
				p.set(EVENT, new Text("LunchTime"));
				p.set("TemporaryDestination", seatsIt.next());
			}
		}
	}
	...
	/**
	 * Finish a demo by changing the event in the involved agents.
	 */
	private void finishDemo() {
		...
		while (peopleIt.hasNext()) {
			...
			if (((Text) p.get(EVENT)).toString().equalsIgnoreCase(
				"DemoTime")) {
				p.set(EVENT, new Text("DemoFinished"));
			}
		}
	}

	/**
	 * Invite as many peolpe as there are seats to the demo.
	 */
	private void organizeDemo() {
		Collection<Place> seats;
		try {
			seats = world.getPlacesOfType("DemoSeat");
		} catch (PlaceNotFoundException e) {
			throw new RuntimeException(e);
		}
		Iterator<Place> seatsIt = seats.iterator();
		...
		while (peopleIt.hasNext()) {
			...
			if (((Text) p.get(TYPE)).toString().equalsIgnoreCase("Staff")
					&& seatsLeft > 0) {
				seatsLeft--;
				p.set(EVENT, new Text("DemoTime"));
				p.set("TemporaryDestination", seatsIt.next());
			}
		}
	}

	/** Plan a meeting at 10h00. */
	private void planDayEvents() {
		...
		lunchStart = new EasyTime(LUNCH_START);
		lunchEnd =
				new EasyTime(lunchStart).shift(LUNCH_DURATION).blur(
					LUNCH_DURATION_BLUR);
		
		demoStart = new EasyTime(DEMO_START);
		demoEnd =
				new EasyTime(demoStart).shift(DEMO_DURATION).blur(
					DEMO_DURATION_BLUR);
