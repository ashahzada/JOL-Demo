...
public abstract class A3JGNode{
	...
	public void setNodeLoc(Location nodeLoc) {
	...
	public ArrayList<A3JGSupervisorRole>  getAllSupervisorRoles() {
		ArrayList<A3JGSupervisorRole> list = new ArrayList<A3JGSupervisorRole>(supervisorRoles.values());
		return list;
	}
	...
	public ArrayList<A3JGFollowerRole>  getAllFollowerRoles() {
		ArrayList<A3JGFollowerRole> list = new ArrayList<A3JGFollowerRole>(followerRoles.values());
		return list;
	}
	...
	public boolean joinGroup(String groupName) throws Exception {
		...
		if (groupInfo.containsKey(groupName)) {
			...
			if (map.get("A3Supervisor") == null) {
				...
				for(int i = 0; i < 	getGroupInfo(groupName).getFollower().size(); i++){
					 folName =getGroupInfo(groupName).getFollower().get(i);
					if (this.getFollowerRole(folName) != null) {
						this.getFollowerRole(folName).setActive(true);
						this.getFollowerRole(folName).setChan(chan);
						this.getFollowerRole(folName).setMap(map);
